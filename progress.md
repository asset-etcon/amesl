...
- **Deploys run through GitHub, not the Vercel CLI.** `vercel[bot]` builds and deploys `main` automatically, so no local Vercel auth is needed to ship. Deployment history read from the GitHub deployments API:
  ...
  - ⬜ **TODO (user):** the site's Vercel project is **not reachable from the `vercel` CLI here.** The CLI is logged in as the *user* `vethanconceptsgroup-7969`, whose only team is `vethan-concepts-technologies`. The production project lives in the **`asset-etcon` Vercel team** — proven by the deployment hostnames (`amesl-<hash>-asset-etcon.vercel.app`; Vercel encodes the owning scope there) and by that team being live at `https://amesl-bmgmlpg7o-asset-etcon.vercel.app`. `vercel project ls --scope asset-etcon` returns `scope-does-not-exist`, which is also what Vercel returns for a scope you are not a member of. Fix by inviting `vethanconceptsgroup-7969` to the asset-etcon team (keeps both scopes, no re-login), or by `vercel login`-ing as an asset-etcon member (drops the vethan-concepts-technologies access on this machine). Note: `vercel link` here created a duplicate `amesl` project under `vethan-concepts-technologies` (`.vercel/project.json` points at it, `orgId team_c9wf2zMSu4mVurtg6gf2S6T2`) which has **zero deployments** — it is not the production project and can be deleted.
- ⬜ **TODO (user, blocking): Vercel Authentication is ON, so the site is not public.** Every anonymous request to the production URL returns **HTTP 200 with `<title>Login - Vercel</title>`** and `X-Vercel-Cache: HIT` — a false-positive "200 OK" that is Vercel's auth interstitial, not the app. Confirmed on `/`, `/products`, `/products/dinnteco/ddce-50plus`, `/contact`, `/admin/login` and on the older `amesl-5466kpwr6-…` deployment, so this has been on the whole time. **A 200 from this host does not prove the app rendered** — always check the `<title>`. Turn it off in the asset-etcon project: Settings → Deployment Protection → Vercel Authentication → Disabled. Until then the app cannot be verified from outside and no visitor can reach it.
  - Consequence: the production build is confirmed green by the deployment status, but the **runtime** (DATABASE_URL reaching Aiven, R2 images rendering) is still unverified in production, because every request is short-circuited by the auth wall. Re-verify once protection is off.
...
- To exercise a server action over HTTP (dev), read the action id from `.next/dev/server/app/<route>/page/server-reference-manifest.json` (`node` → the 40-hex key), POST to the page URL with header `Next-Action: <id>` and `Accept: text/x-component`, and encode the body with `encodeReply([arg])` from `next/dist/compiled/react-server-dom-webpack/client.edge`. A hand-written JSON body is **not** flight-encoded and decodes to `undefined`.
